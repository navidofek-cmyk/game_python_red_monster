...
def test_spawn_area_produces_platforms_and_enemies():
    platforms, enemies, bg, decor, items = spawn_area(START_COORD, Platform, Enemy)
    assert len(platforms) > 0
    assert isinstance(bg, tuple) and len(bg) == 3


def test_spawn_area_goal_has_altar_decor():
    *_, decor, _ = spawn_area(GOAL_COORD, Platform, Enemy)
    assert any(kind == "altar" for (kind, _, _) in decor)


def test_spawn_area_with_item_class_builds_items():
    from items import Item
    _, _, _, _, items = spawn_area(START_COORD, Platform, Enemy, Item)
    assert len(items) >= 1
    assert all(hasattr(it, "uid") for it in items)


def test_spawn_area_respects_collected_set():
    from items import Item
    _, _, _, _, before = spawn_area(START_COORD, Platform, Enemy, Item)
    collected = {before[0].uid}
    _, _, _, _, after = spawn_area(
        START_COORD, Platform, Enemy, Item, collected=collected)
