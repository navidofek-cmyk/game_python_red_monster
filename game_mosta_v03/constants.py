...
# Player HP + power-ups
MAX_HP:               Final[int]   = 3
INVINCIBILITY_FRAMES: Final[int]   = FPS * 1
BOOST_FRAMES:         Final[int]   = FPS * 5
SPEED_BOOST_MULT:     Final[float] = 1.6
JUMP_BOOST_MULT:      Final[float] = 1.25
...
class ItemKind(StrEnum):
    """Pickable item types (hotkeys 1..4 use them in order)."""
    POTION = "potion"   # +1 HP (bounded by MAX_HP)
    SPEED  = "speed"    # temporary speed boost
    JUMP   = "jump"     # temporary jump boost
    KEY    = "key"      # reserved for locked areas in a future version


ITEM_HOTKEYS: Final[tuple[ItemKind, ...]] = (
    ItemKind.POTION, ItemKind.SPEED, ItemKind.JUMP, ItemKind.KEY,
)
