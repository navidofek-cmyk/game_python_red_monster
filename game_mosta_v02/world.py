...
from constants import SCREEN_W, SCREEN_H, BIOMES, AiType, BiomePalette
...
def spawn_area(coord: Coord, Platform, Enemy, entry_from: str | None = None, player=None):
    """Instantiate platforms and enemies for an area.

    ``Platform`` and ``Enemy`` are injected as dependencies so this function
    stays easy to unit-test without pygame (tests pass real classes that accept
    the same arguments). This is a common Python idiom: pass the class, not
    import it inside.
    """
    defn      = AREAS[coord]
    ...
    return platforms, enemies, palette.bg, decor
